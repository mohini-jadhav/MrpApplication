...
class ProjectSettings {
	...
	function setPage($page) {
		...
		if ($this->isPageTypeForView ( $page )) {
			$tableType = $this->getTableType ();
			if ($tableType != "report" && $tableType != "chart" && ($page == PAGE_CHART || $page == PAGE_REPORT))
	 ...
	 * 
	 * @param
	 *        	{string} table wich contain a lookup field
	 * @param
	 *        	{string} name of lookup field
	 * @param
	 *        	{int} index
	 */
	...
	function getCustomExpressionIndex($mainTable, $mainField) {
		if (! $this->isExistsTableKey ( ".customExpressionIndexes" ))
			$this->_tableData [".customExpressionIndexes"] = array ();
		if (isset ( $this->_tableData [".customExpressionIndexes"] [$mainTable] ) && isset ( $this->_tableData [".customExpressionIndexes"] [$mainTable] [$mainField] ))
	 ...
	 * 
	 * @param string $tName
	 *        	- it's data source detail table name
	 ...
	 *        	- it's detail data sourse table name,
	 * @return array if success otherwise false
	 */
	 ...
	 * 
	 * @param string $tName
	 *        	- it's data source master table name
	 * @return array if success otherwise false
	 */
	 ...
	 *        	- it's master table name,
	 * @param string $tName
	 *        	- it's current (detail) table
	 ...
	 * 
	 * @param string $table
	 *        	- current data source table name
	 * @return array
	 */
	function getAddTabs() {
	 ...
	 *        	{string} field name
	 * @param
	 *        	{array} file info (name, type, size)
	 */
...
function GetTableURL($table = "") {
	...
	if (! $table)
		$table = $strTableName;
	if ("customer_header" == $table)
		return "customer_header";
	if ("customer_platform" == $table)
		return "customer_platform";
	if ("customer_stage" == $table)
		return "customer_stage";
	if ("customer_resource_allocation" == $table)
		return "customer_resource_allocation";
	if ("employee_auth" == $table)
		return "employee_auth";
	if ("employee_header" == $table)
	...
	if ("admin_users" == $table)
		return "admin_users";
	if ("customerallocation" == $table)
		return "customerallocation";
	if ("backup" == $table)
		return "backup";
	if ("database" == $table)
		return "database";
	if ("application" == $table)
		return "application";
	if ("stage" == $table)
		return "stage";
	if ("CUST_ALLOC" == $table)
	...
	if ("calandar" == $table)
		return "calandar";
	if ("MRP Dashboard" == $table)
		return "MRP_Dashboard";
	if ("allocation" == $table)
		return "allocation";
	if ("View" == $table)
		return "View";
	if ("skillstable" == $table)
		return "skillstable";
	if ("employeeskills" == $table)
		return "employeeskills";
	if ("skilllevel" == $table)
		return "skilllevel";
	if ("category" == $table)
		return "category";
	if ("customer_Geotag" == $table)
		return "customer_Geotag";
	if ("tbl_test center" == $table)
		return "tbl_test_center";
	if ("tbl_director" == $table)
		return "tbl_director";
	if ("stage2" == $table)
		return "stage2";
	if ("tbl_categorytechnology" == $table)
		return "tbl_categorytechnology";
	if ("tbl_technologycustomer" == $table)
		return "tbl_technologycustomer";
	if ("customertechnology" == $table)
		return "customertechnology";
	if ("tbl_role" == $table)
		return "tbl_role";
	if ("sizecategory" == $table)
		return "sizecategory";
	if ("Workload" == $table)
		return "Workload";
	if ("time" == $table)
	...
	if ("color" == $table)
		return "color";
	if ("customerallocation1" == $table)
		return "customerallocation1";
	if ("tbl_director1" == $table)
		return "tbl_director1";
	if ("tbl_director3" == $table)
		return "tbl_director3";
	if ("size" == $table)
		return "size";
	if ("tbl_allocation_input" == $table)
		return "tbl_allocation_input";
	if ("Allocation Dashboard New" == $table)
		return "Allocation_Dashboard_New";
	if ("tbl_resource_allocation" == $table)
		return "tbl_resource_allocation";
	
	if ("engagementstat" == $table)
...
function GetTableByShort($shortTName = "") {
	...
	if (! $shortTName)
		return false;
	if ("customer_header" == $shortTName)
		return "customer_header";
	if ("customer_platform" == $shortTName)
		return "customer_platform";
	if ("customer_stage" == $shortTName)
		return "customer_stage";
	if ("customer_resource_allocation" == $shortTName)
		return "customer_resource_allocation";
	if ("employee_auth" == $shortTName)
		return "employee_auth";
	if ("employee_header" == $shortTName)
		return "employee_header";
	if ("lookup" == $shortTName)
		return "lookup";
	...
	if ("admin_users" == $shortTName)
		return "admin_users";
	if ("customerallocation" == $shortTName)
		return "customerallocation";
	if ("backup" == $shortTName)
		return "backup";
	if ("database" == $shortTName)
		return "database";
	if ("application" == $shortTName)
		return "application";
	if ("stage" == $shortTName)
		return "stage";
	if ("CUST_ALLOC" == $shortTName)
		return "CUST_ALLOC";
	if ("calandar" == $shortTName)
		return "calandar";
	if ("MRP_Dashboard" == $shortTName)
		return "MRP Dashboard";
	if ("allocation" == $shortTName)
		return "allocation";
	if ("View" == $shortTName)
		return "View";
	if ("skillstable" == $shortTName)
		return "skillstable";
	if ("employeeskills" == $shortTName)
		return "employeeskills";
	if ("skilllevel" == $shortTName)
		return "skilllevel";
	if ("category" == $shortTName)
		return "category";
	if ("customer_Geotag" == $shortTName)
		return "customer_Geotag";
	if ("tbl_test_center" == $shortTName)
		return "tbl_test center";
	if ("tbl_director" == $shortTName)
		return "tbl_director";
	if ("stage2" == $shortTName)
		return "stage2";
	if ("tbl_categorytechnology" == $shortTName)
		return "tbl_categorytechnology";
	if ("tbl_technologycustomer" == $shortTName)
		return "tbl_technologycustomer";
	if ("customertechnology" == $shortTName)
		return "customertechnology";
	if ("tbl_role" == $shortTName)
		return "tbl_role";
	if ("sizecategory" == $shortTName)
		return "sizecategory";
	if ("Workload" == $shortTName)
		return "Workload";
	if ("time" == $shortTName)
		return "time";
	if ("tbl_sme" == $shortTName)
		return "tbl_sme";
	if ("emptime" == $shortTName)
		return "emptime";
	if ("timezone" == $shortTName)
		return "timezone";
	if ("color" == $shortTName)
		return "color";
	if ("customerallocation1" == $shortTName)
		return "customerallocation1";
	if ("tbl_director1" == $shortTName)
		return "tbl_director1";
	if ("tbl_director3" == $shortTName)
		return "tbl_director3";
	if ("size" == $shortTName)
		return "size";
	if ("tbl_allocation_input" == $shortTName)
		return "tbl_allocation_input";
	if ("Allocation_Dashboard_New" == $shortTName)
		return "Allocation Dashboard New";
	if ("tbl_resource_allocation" == $shortTName)
		return "tbl_resource_allocation";
	
	if ("engagementstat" == $shortTName)
		return "engagementstat";
}
