...
class EditPage extends RunnerPage {
	...
	protected function getAfterEditAction() {
		...
		if ($this->mode == EDIT_POPUP && $this->pSet->checkClosePopupAfterEdit () || $action == AE_TO_VIEW && ! $this->viewAvailable () || $action == AE_TO_NEXT_EDIT && ! count ( $this->getNextKeys () ) || $action == AE_TO_PREV_EDIT && ! count ( $this->getPrevKeys () )) {
			$action = AE_TO_LIST;
		}
	...
	protected function doCommonAssignments() {
		...
		foreach ( $this->keyFields as $i => $k ) {
			...
			if ($viewFormat == FORMAT_HTML || $viewFormat == FORMAT_FILE_IMAGE || $viewFormat == FORMAT_FILE || $viewFormat == FORMAT_HYPERLINK || $viewFormat == FORMAT_HYPERLINK || $viewFormat == FORMAT_EMAILHYPERLINK || $viewFormat == FORMAT_CHECKBOX) {
	...
	protected function prepareDetailsTables() {
		...
		$this->jsSettings ['tableSettings'] [$this->tName] ['dpParams'] = array (
				'tableNames' => $dpParams ['strTableNames'],
				'ids' => $dpParams ['ids'] 
		);
	...
	protected function prepareReadonlyFields() {
		...
		foreach ( $fields as $f ) {
			if ($this->pSet->getEditFormat ( $f ) == EDIT_FORMAT_READONLY && ($this->pSet->appearOnEditPage ( $f ) || $this->pSet->appearOnInlineEdit ( $f )))
		...
		foreach ( $newData as $fName => $val ) // .net compatibility issue
{
	...
	protected function callAfterEditEvent() {
		...
		$this->eventsObject->AfterEdit ( $this->newRecordData, $this->getWhereClause ( false ), $this->getOldRecordData (), $this->keys, $this->mode == EDIT_INLINE, $this );
	}
	...
	protected function callBeforeEditEvent() {
		...
		$usermessage = "";
		$ret = $this->eventsObject->BeforeEdit ( $this->newRecordData, $this->getWhereClause ( true ), $this->getOldRecordData (), $this->oldKeys, $usermessage, $this->mode == EDIT_INLINE, $this );
	...
	protected function callCustomEditEvent() {
		...
		$usermessage = "";
		$ret = $this->eventsObject->CustomEdit ( $this->newRecordData, $this->getWhereClause ( true ), $this->getOldRecordData (), $this->oldKeys, $usermessage, $this->mode == EDIT_INLINE, $this );
	...
	protected function SecurityRedirect() {
		if ($this->mode == EDIT_INLINE) {
			echo printJSON ( array (
					"success" => false,
					"message" => "The record is not editable" 
			) );
			exit ();
		}
