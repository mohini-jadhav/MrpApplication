...
class RightsPage extends ListPage {
	...
	function fillGroupsArr() {
		...
		$sql = "select " . $grConnection->addFieldWrappers ( "GroupID" ) . ", " . $grConnection->addFieldWrappers ( "Label" ) . " from " . $grConnection->addTableWrappers ( "codis_uggroups" ) . " order by " . $grConnection->addFieldWrappers ( "Label" );
	...
	function getRights() {
		...
		$sql = "select " . $this->connection->addFieldWrappers ( "GroupID" ) . ", " . $this->connection->addFieldWrappers ( "TableName" ) . ", " . $this->connection->addFieldWrappers ( "AccessMask" ) . " from " . $this->connection->addTableWrappers ( "codis_ugrights" ) . " order by " . $this->connection->addFieldWrappers ( "GroupID" );
	 ...
	 * 
	 * @param array $rowInfoArr
	 *        	array with total info, that assignes grid
	 */
	 ...
	 * I.e.
	 * $mask = "ADE", $possibleMask = "AESP", return "AE"
	 */
	...
	function updateTablePermissions($table, $group, $mask) {
		$rightWTableName = $this->connection->addTableWrappers ( "codis_ugrights" );
		...
		if ($data) {
			...
			$sql = "insert into " . $rightWTableName . " (" . $groupisWFieldName . ", " . $tableNameWFieldName . ", " . $accessMaskWFieldName . ")" . " values (" . $group . ", " . $this->connection->prepareString ( $table ) . ", '" . $mask . "')";
		}
