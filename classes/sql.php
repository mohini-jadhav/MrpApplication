...
class SQLField extends SQLEntity {
	...
	function toSql($query) {
		...
		if ($query->cipherer != null) {
			return $query->cipherer->GetFieldName ( ($this->m_strTable != "" && $query->TableCount () > 1 ? $this->connection->addTableWrappers ( $this->m_strTable ) . "." : "") . $this->connection->addFieldWrappers ( $this->m_strName ) );
		}
...
class SQLQuery extends SQLEntity {
	...
	function AddCustomExpression($expression, $pSet, $mainTable, $mainFiled, $alias = "") {
		...
		if (! $itemFound)
			$this->m_fieldlist [] = new SQLFieldListItem ( array (
					"m_expr" => $expression,
					"m_alias" => $alias,
	...
	function gSQLWhere_having_fromQuery($sqlWhereExpr, $where = "", $having = "", $criteria = "or") {
		return SQLQuery::gSQLWhere_having ( $this->HeadToSql (), $this->FromToSql (), whereAdd ( $this->WhereToSql (), $sqlWhereExpr ), $this->GroupByToSql (), $this->Having ()->toSql ( $this ), $where, $having, $criteria );
	}
	 ...
	 * @param
	 *        	String having (optional)
	 * @param
	 *        	String searchCriteria (optional)
	 */
	public function gSQLRowCount($where, $connection, $having = "", $searchCriteria = "or") {
		return SQLQuery::gSQLRowCount_int ( $this->HeadToSql (), $this->FromToSql (), $this->WhereToSql (), $this->GroupByToSql (), $this->Having ()->toSql ( $this ), $where, $having, $connection, $searchCriteria );
	}
