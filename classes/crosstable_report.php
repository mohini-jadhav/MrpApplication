...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("customer_header" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customer_platform" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customer_stage" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customer_resource_allocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("employee_auth" == $this->xml_array['tables'][0])
			$isDB = true;
		if("employee_header" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lookup" == $this->xml_array['tables'][0])
			$isDB = true;
		if("codis_ugrights" == $this->xml_array['tables'][0])
			$isDB = true;
		...
		if("users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customerallocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("backup" == $this->xml_array['tables'][0])
			$isDB = true;
		if("database" == $this->xml_array['tables'][0])
			$isDB = true;
		if("application" == $this->xml_array['tables'][0])
			$isDB = true;
		if("stage" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customerallocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("calandar" == $this->xml_array['tables'][0])
			$isDB = true;
		if("" == $this->xml_array['tables'][0])
			$isDB = true;
		if("allocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customerallocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("skillstable" == $this->xml_array['tables'][0])
			$isDB = true;
		if("employeeskills" == $this->xml_array['tables'][0])
			$isDB = true;
		if("skilllevel" == $this->xml_array['tables'][0])
			$isDB = true;
		if("category" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customer_header" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_test center" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_director" == $this->xml_array['tables'][0])
			$isDB = true;
		if("stage2" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_categorytechnology" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_technologycustomer" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customertechnology" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_role" == $this->xml_array['tables'][0])
			$isDB = true;
		if("sizecategory" == $this->xml_array['tables'][0])
			$isDB = true;
		if("customerallocation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("time" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_sme" == $this->xml_array['tables'][0])
			$isDB = true;
		if("emptime" == $this->xml_array['tables'][0])
			$isDB = true;
		if("timezone" == $this->xml_array['tables'][0])
			$isDB = true;
		if("color" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_director" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_director" == $this->xml_array['tables'][0])
			$isDB = true;
		if("size" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_allocation_input" == $this->xml_array['tables'][0])
			$isDB = true;
		if("" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tbl_resource_allocation" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
