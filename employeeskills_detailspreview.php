...
require_once ("include/employeeskills_variables.php");
...
$page_layouts ["employeeskills_detailspreview"] = $layout;
...
if ($mastertable == "employee_header") {
	$where = "";
	$formattedValue = make_db_value ( "EmployeeID", $_SESSION [$sessionPrefix . "_masterkey1"] );
	if ($formattedValue == "null")
		$where .= $pageObject->getFieldSQLDecrypt ( "EmployeeID" ) . " is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt ( "EmployeeID" ) . "=" . $formattedValue;
}
...
if ($rowcount) {
	...
	while ( $data && $recordsCounter < $display_count ) {
		...
		$row = array ();
		$keylink = "";
		$keylink .= "&key1=" . runner_htmlspecialchars ( rawurlencode ( @$data ["AutEskillID"] ) );
		
		// PlatformType -
		$viewContainer->recId = $recordsCounter;
		$value = $viewContainer->showDBValue ( "PlatformType", $data, $keylink );
		$row ["PlatformType_value"] = $value;
		$format = $pSet->getViewFormat ( "PlatformType" );
		...
		if ($format == FORMAT_NUMBER || IsNumberType ( $pSet->getFieldType ( "PlatformType" ) ))
			$class = ' rnr-field-number';
		$row ["PlatformType_class"] = $class;
		// PlatformName -
		$viewContainer->recId = $recordsCounter;
		...
		// Certificate - HTML
		$viewContainer->recId = $recordsCounter;
		$value = $viewContainer->showDBValue ( "Certificate", $data, $keylink );
		$row ["Certificate_value"] = $value;
		$format = $pSet->getViewFormat ( "Certificate" );
		$class = "rnr-field-text";
		if ($format == FORMAT_FILE)
			$class = ' rnr-field-file';
		if ($format == FORMAT_AUDIO)
		...
		if ($format == FORMAT_CHECKBOX)
			$class = ' rnr-field-checkbox';
		if ($format == FORMAT_NUMBER || IsNumberType ( $pSet->getFieldType ( "Certificate" ) ))
			$class = ' rnr-field-number';
		$row ["Certificate_class"] = $class;
		// Comments - HTML
		$viewContainer->recId = $recordsCounter;
...
$xt->load_template ( GetTemplateName ( "employeeskills", "detailspreview" ) );
