...
$layout = new TLayout ( "login2", "FusionAqua", "MobileAqua" );
$layout->version = 2;
...
if ($pageObject->auditObj) {
	if ($pageObject->auditObj->LoginAccess ()) {
		$logacc = false;
		$message = mysprintf ( "Access denied for %s minutes", array (
				$pageObject->auditObj->LoginAccess () 
		) );
	}
}
