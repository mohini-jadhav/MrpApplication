...
require_once("include/tbl_technologycustomer_variables.php");
...
$page_layouts["tbl_technologycustomer_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["PlatformID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["PlatformID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
