...
require_once("include/sizecategory_variables.php");
...
$page_layouts["sizecategory_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["StagecatID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["StagecatID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
