...
require_once("include/tbl_test_center_variables.php");
...
$page_layouts["tbl_test_center_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["AutCenterID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["AutCenterID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
