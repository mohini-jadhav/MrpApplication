...
require_once("include/tbl_categorytechnology_variables.php");
...
$page_layouts["tbl_categorytechnology_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["TechnologyCategoryID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["TechnologyCategoryID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
