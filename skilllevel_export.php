...
require_once("include/skilllevel_variables.php");
...
$page_layouts["skilllevel_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["AutSkillId"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["AutSkillId"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
